                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Edit Clinic Configuration</h3>
                        </div>

                        <div class="card-body">
                            <form role="form" class="row" action="{{ route('clinicConfigurationUpdate') }}"
                                  method="post"
                                    ...
                                    <div class="col-sm-12 col-md-6 col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title" id="title"
                                                   ...
                                                   placeholder="Enter Name" value="{{ $clinicConfig->key }}" required>
                                            @if ($errors->has('title'))
                                    ...
                                    <div class="col-sm-12 col-md-6 col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <select name="clinic_id" id="clinic_id"
                                                    class="form-control {{ $errors->has('clinic_id') ? ' is-invalid' : '' }}"
                                                    title="Select Clinic" required>
                                                @foreach($clinics as $clinic)
                                                    <option
                                                        value="{{ $clinic->id }}" {{($clinicConfig->clinic_id ==$clinic->id )? "selected":''}} >{{ $clinic->name }}</option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select name="lab_tests[]" id="lab_tests" multiple="multiple"
                                                ...
                                                @foreach($labTests as $labTest)
                                                    <option value="{{ $labTest->id }}"
                                                            @foreach($clinicConfig->latTests as $item)
                                            ...
                                            <select name="facilities[]" id="facilities" multiple="multiple"
                                                ...
                                                @foreach($facilities as $facility)
                                                    <option value="{{ $facility->id }}"
                                                            @foreach($clinicConfig->facilities as $item)
                                            ...
                                            <select name="medications[]" id="medications" multiple="multiple"
                                                ...
                                                @foreach($medications as $medication)
                                                    <option value="{{ $medication->id }}"
                                                            @foreach($clinicConfig->medications as $item)
                                            ...
                                            <select name="specialties[]" id="specialties" multiple="multiple"
                                                ...
                                                @foreach($specialties as $specialty)
                                                    <option value="{{ $specialty->id }}"
                                                            @foreach($clinicConfig->specialties as $item)
                                ...
                                <div class="col-lg-12">
                                    <input type="hidden" name="clinic_config_id" value="{{$clinicConfig->id}}">
...
@section('scripts')
    <script src="{{asset('public/admin/js/clinicConfiguration.js')}}"></script>
@endsection
