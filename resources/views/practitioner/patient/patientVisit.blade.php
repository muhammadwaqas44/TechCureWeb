                                        ...
                                        <div class="start-video">
                                            <div class="row">
                                                <div class="col-md-6 p-0">
                                                    <a href="{{route('practitionerJoinAppointment',[$appointment->id])}}"
                                ...
                                <div class="tab-content">
                                    ...
                                    <div id="labs" class="container tab-pane">
                                        ...
                                        @if(count($patient->reports->where('type',0)) > 0)
                                            <div class="owl-carousel owl-carousel-slider">
                                                @foreach($patient->reports->where('type',0)->sortByDesc('created_at') as $report)
                                    ...
                                    <div id="invoices" class="container tab-pane fade">
                                        ...
                                        @if(count($patient->reports->where('type', 1)) > 0)
                                            <div class="owl-carousel owl-carousel-slider">
                                                @foreach($patient->reports->where('type', 1)->sortByDesc('created_at') as $report)
                                                    @php
                                                        $name = $report->image_url;
                                                            $pi = pathinfo($name);
                                                            $txt = $pi['filename'];
                                    ...
                                    <div id="complaints" class="container tab-pane fade">
                                        ...
                                        @if(count($patient->reports->where('type', 2)) > 0)
                                            <div class="owl-carousel owl-carousel-slider">
                                                @foreach($patient->reports->where('type', 2)->sortByDesc('created_at') as $report)
                                                    @php
                                                        $name = $report->image_url;
                                                            $pi = pathinfo($name);
                                                            $txt = $pi['filename'];
                                    ...
                                    <div id="p_prescription" class="container tab-pane fade">
                                        ...
                                        @if(count($patient->reports->where('type', 3)) > 0)
                                            <div class="owl-carousel owl-carousel-slider">
                                                @foreach($patient->reports->where('type', 3)->sortByDesc('created_at') as $report)
                                                    @php
                                                        $name = $report->image_url;
                                                            $pi = pathinfo($name);
                                                            $txt = $pi['filename'];
                                    ...
                                    <div id="sugar_chart" class="container tab-pane fade">
                                        ...
                                        @if(count($patient->reports->where('type', 4)) > 0)
                                            ...
                                            <div class="owl-carousel owl-carousel-slider">
                                                @foreach($patient->reports->where('type', 4)->sortByDesc('created_at') as $report)
                                                    @php
                                                        $name = $report->image_url;
                                                            $pi = pathinfo($name);
                                                            $txt = $pi['filename'];
                                    ...
                                    <div id="vitals" class="container tab-pane fade">
                                        ...
                                        @if(count($patient->reports->where('type', 5)) > 0)
                                            <div class="owl-carousel owl-carousel-slider">
                                                @foreach($patient->reports->where('type', 5)->sortByDesc('created_at') as $report)
                                                    @php
                                                        $name = $report->image_url;
                                                            $pi = pathinfo($name);
                                                            $txt = $pi['filename'];
                                ...
                                <div align="right">
                                    <div class="row">
                                        ...
                                        <a type="button" class="send-pateint-btn mr-2"
                                           href="{{ route('sendToPatientReport', ['patientId' => $patient->id, 'appointmentId' => $appointment->id, 'practitionerId' => Auth::guard('practitioner')->user()->id]) }}">Save
                                            & Preview</a>
...
@section('scripts')
    ...
    <script>
        ...
        //     $('.zoom-image').imageZoom({zoom: 300});
        // });
        {{--                    @foreach($patient->reports as $report)--}}
        {{--                    $('#zoom-image{{$report->id}}').imageZoom({zoom: 300});--}}
        {{--                    @endforeach--}}
        {{--            @foreach($patient->reports as $report)--}}
        {{--            @php--}}
        {{--                $deepzoom = Jeremytubbs\Deepzoom\DeepzoomFactory::create([--}}
        {{--                'path' => "{{asset('storage/app/public/'.$report->image_url)}}", // Export path for tiles--}}
        {{--                'driver' => 'imagick', // Choose between gd and imagick support.--}}
        {{--                'format' => 'jpg',--}}
        {{--                ]);--}}
        {{--                // folder, file are optional and will default to filename--}}
        {{--                $response = $deepzoom->makeTiles("{{asset('storage/app/public/'.$report->image_url)}}", 'file', 'folder');--}}
        {{--            @endphp--}}
        {{--            var viewer = OpenSeadragon({--}}
        {{--                    id: "zoom-image{{$report->id}}",--}}
        {{--                    prefixUrl: "/openseadragon/images/",--}}
        {{--                    tileSources: "{{$response['data']['DZI']}}"--}}
        {{--                });--}}
    ...
    <script>
        ...
        function checkPatientStatus() {
            $.ajax({
                method: "GET",
                url: "{{url('practitioner/checkPatientStatus')}}",
                success: function (response) {
