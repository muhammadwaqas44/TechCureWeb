...
class AppointmentController extends Controller
{
    ...
    public function todayAppointment()
    {
        ...
        return view('practitioner.appointment.index', ['appointments' => $appointments, 'title' => 'Appointments']);
    }
    ...
    public function index(Request $request)
    {
        ...
        if (empty($request->search_date) && empty($request->status) && empty($request->payment)) {
            $appointments = Appointment::where('practitioner_id', Auth::guard('practitioner')->user()->id)->where('date', $todayOnly)
                ->orderBy('date', 'DESC');
        } else {
            $appointments = Appointment::where('practitioner_id', Auth::guard('practitioner')->user()->id)
                ->orderBy('date', 'DESC');
        ...
        $appointmentsCount = Appointment::where('practitioner_id', Auth::guard('practitioner')->user()->id)->count();
        $completedCount = Appointment::where(['practitioner_id' => Auth::guard('practitioner')->user()->id, 'status' => 5])->count();

        return view('practitioner.appointment.index', ['appointments' => $appointments, 'title' => 'Appointments', 'appointmentsCount' => $appointmentsCount, 'completedCount' => $completedCount]);
    }
    ...
    public function checkPatientStatus()
    {
        ...
        $appointments = Appointment::where('practitioner_id', (int)$practitioner->id)->get();
    ...
    public function create()
    {
        ...
        $clinics = $practitioner->getClinics->unique('id');

        $patients = Patient::where('status', 1)->get();

        $patientTypes = PatientType::where('status', 1)->get();

        return view('practitioner.appointment.create', ['clinics' => $clinics, 'patients' => $patients, 'patientTypes' => $patientTypes, 'title' => 'Create Appointment']);
    }
    ...
    public function store(Request $request)
    {
        ...
        $dataInvoice = [
            ...
            'patient_visit_no' => $patientVisit->visit_number,
            'date' => Carbon::parse($appointment->date)->format('d F Y') . ' ' . $appointment->time_slot,
            'amount' => $appointment->payment->amount,
            'mod' => $appointment->payment->payment_method,
        ];
        $generatePDF = PDF::loadView('practitioner.invoice', array('dataInvoice' => $dataInvoice));

        $filename = $appointment->id . '.pdf';
        $pdf = $generatePDF->save('storage/app/public/reportImages/' . $filename);

        if ($pdf) {
            $data = [
                'patient_id' => $patient->id,
                'title' => $appointment->id . '-Invoice',
                'type' => 1,
            ];
            $patientReport = PatientReport::create($data);

            if ($filename) {
                $imageUrl = 'reportImages/' . $filename;
...
//        event(new Notify($notification->id, $notification->title, $notification->message, $currentTime, $notification->user_type, $notification->user_id));
        ...
        return redirect()->route('practitionerAppointmentList')->with('success-message', 'Record Added Successfully.');
    }
    ...
    public function edit($id)
    {
        ...
        return view('practitioner.appointment.edit', ['title' => 'Edit Appointment', 'appointment' => $appointment, 'patients' => $patients, 'patientTypes' => $patientTypes, 'clinics' => $clinics, 'time_slots' => $time_slots]);
    }
...
//        event(new Notify($notification->id, $notification->title, $notification->message, $currentTime, $notification->user_type, $notification->user_id));
        ...
        return redirect()->route('practitionerAppointmentList')->with('success-message', 'Record Updated Successfully.');
    }
    ...
    public function getTimeSlots(Request $request)
    {
        ...
        $practitionerClinics = PractitionerClinics::where('practitioner_id', $practitioner->id)
            ->where('clinic_id', $request->clinic_id)
            ->get();


        $practitionerClinicDays = array();
        foreach ($practitionerClinics as $practitionerClinic) {
            $practitionerClinicDays[] = PractitionerClinicDay::where('practitioner_clinic_id', $practitionerClinic->id)
                ->get()
                ->pluck('day')
                ->toArray();
        }
        $practitionerClinicDaysFinal = [];
        foreach ($practitionerClinicDays as $key => $practitionerClinicDay) {
            if (isset($practitionerClinicDays[$key + 1])) {
                $finalArray1 = $practitionerClinicDays[$key + 1];
            } else {
                $finalArray1 = [];
            }
            if (isset($practitionerClinicDays[$key + 2])) {
                $finalArray2 = $practitionerClinicDays[$key + 2];
            } else {
                $finalArray2 = [];
            }
            if (isset($practitionerClinicDays[$key + 3])) {
                $finalArray3 = $practitionerClinicDays[$key + 3];
            } else {
                $finalArray3 = [];
            }
            if (isset($practitionerClinicDays[$key + 4])) {
                $finalArray4 = $practitionerClinicDays[$key + 4];
            } else {
                $finalArray4 = [];
            }
            if (isset($practitionerClinicDays[$key + 5])) {
                $finalArray5 = $practitionerClinicDays[$key + 5];
            } else {
                $finalArray5 = [];
            }
            if (isset($practitionerClinicDays[$key + 6])) {
                $finalArray6 = $practitionerClinicDays[$key + 6];
            } else {
                $finalArray6 = [];
            }
            if (isset($practitionerClinicDays[$key + 7])) {
                $finalArray7 = $practitionerClinicDays[$key + 7];
            } else {
                $finalArray7 = [];
            }
            if (isset($practitionerClinicDays[$key + 8])) {
                $finalArray8 = $practitionerClinicDays[$key + 8];
            } else {
                $finalArray8 = [];
            }
            if (isset($practitionerClinicDays[$key + 9])) {
                $finalArray9 = $practitionerClinicDays[$key + 9];
            } else {
                $finalArray9 = [];
            }
            if (isset($practitionerClinicDays[$key + 10])) {
                $finalArray10 = $practitionerClinicDays[$key + 10];
            } else {
                $finalArray10 = [];
            }
            $practitionerClinicDaysFinal[] = array_merge($practitionerClinicDay, $finalArray1, $finalArray2, $finalArray3, $finalArray4, $finalArray5, $finalArray6, $finalArray7, $finalArray8, $finalArray9, $finalArray10);
        }


        if (!in_array($day, $practitionerClinicDaysFinal[0], true)) {
            return ['status' => 0, 'error' => 'Practitioner is not available in this clinic on your selected date.'];
        }
        $time_slots = array();
        foreach ($practitionerClinics as $practitionerClinic) {
            $practitionerClinicDaysFor = PractitionerClinicDay::where('practitioner_clinic_id', $practitionerClinic->id)
                ->get()
                ->pluck('day')
                ->toArray();

            if (in_array($day, $practitionerClinicDaysFor, true)) {
                $timeIn = Carbon::parse($practitionerClinic->from_time);
        ...
        foreach ($time_slots as $key => $value){
            $ord[] = strtotime($value['value']);
        }
        array_multisort($ord, SORT_ASC, $time_slots);
    ...
    public function getPractitionerFee(Request $request)
    {
        if (empty($request->clinic_id) || empty($request->patient_type_id)) {
            return ['status' => 0, 'error' => 'Patient Type & Clinic must be Selected First.'];
        }
        ...
        if ($patientType != null) {
            ...
            if ($request->type == 0) {
                ...
                $practitionerClinicFee = PractitionerClinics::select('online_fee')->where('practitioner_id', Auth::guard('practitioner')->user()->id)->where('clinic_id', $request->clinic_id)->first();
