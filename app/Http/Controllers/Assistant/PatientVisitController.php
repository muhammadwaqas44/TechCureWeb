...
class PatientVisitController extends Controller
{
    public function manageAppointment(Request $request, $patientId, $appointmentId, $practitionerId)
    {
        ...
        $referalDoctors = Practitioner::where([['id', '!=', $practitionerId], ['status', 1]])->get();
        ...
        $patientVisitPrevious = PatientVisit::whereHas('appointment', function ($q) {
            $q->where('status', 5);
...
//        dd($request->all());
    ...
    {
        if ($request->pulse > 220) {
            return response()->json(['result' => 'error', 'message' => 'Pulse must be less than 220']);
        }
    ...
    {
        if ($request->height_in > 120) {
            return response()->json(['result' => 'error', 'message' => 'Height in Inches must be less than 120']);
        }
    ...
    public function heightCmsPatientVisit(Request $request)
    {
        ...
        if ($patientVital == null) {
            ...
            $patientVital->update(['height_cms' => $request->height_cms, 'height_ft' => null, 'height_in' => null]);
        }
    ...
    public function heightCms2PatientVisit(Request $request)
    {
        ...
        if ($patientVital == null) {
            $patientVitalData = [
                ...
                'patient_id' => $patient->id,
                'patient_name' => $patient->name,
                'height_cms_2' => $request->height_cms_2,
            ];

            $patientVitalStore = PatientVital::create($patientVitalData);
        } else {
            $patientVital->update(['height_cms_2' => $request->height_cms_2, 'height_ft_2' => null, 'height_in_2' => null]);
        }
        return response()->json(['result' => 'success', 'message' => 'Height in Centimeters Added Successfully!']);
    }
...
// dd($request->all());
    ...
    public function sendToPatientReport(Request $request, $patientId, $appointmentId, $practitionerId)
    {
        ...
        $generatePDF = PDF::loadView('practitioner.patient.patientVisitReportPDF', array('patient' => $patient, 'patientVisit' => $patientVisit, 'practitioner' => $practitioner, 'patientVitals' => $patientVitals, 'patientVisitPrescription' => $patientVisitPrescription, 'patientLabTests' => $patientLabTests, 'patientSugarChart' => $patientSugarChart, 'patientRXMedicines' => $patientRXMedicines, 'patientPastMedicalHistories' => $patientPastMedicalHistories, 'patientAdrs' => $patientAdrs, 'patientReferPractitioners' => $patientReferPractitioners));
...
//        }
        ...
        return view('practitioner.patient.patientVisitReport', ['patient' => $patient, 'patientVisit' => $patientVisit, 'practitioner' => $practitioner, 'patientVitals' => $patientVitals, 'patientVisitPrescription' => $patientVisitPrescription, 'patientLabTests' => $patientLabTests, 'patientSugarChart' => $patientSugarChart, 'patientRXMedicines' => $patientRXMedicines, 'patientPastMedicalHistories' => $patientPastMedicalHistories, 'patientAdrs' => $patientAdrs]);
    }
...
//        $appointment->where('id', $appointment->id)->update(['status' => 5]);
        ...
        return view('practitioner.patient.patientVisitReportPreview', ['patient' => $patient, 'patientVisit' => $patientVisit, 'practitioner' => $practitioner, 'patientVitals' => $patientVitals, 'patientVisitPrescription' => $patientVisitPrescription, 'patientLabTests' => $patientLabTests, 'patientSugarChart' => $patientSugarChart, 'patientRXMedicines' => $patientRXMedicines, 'patientPastMedicalHistories' => $patientPastMedicalHistories, 'patientAdrs' => $patientAdrs, 'patientReferPractitioners' => $patientReferPractitioners]);
    }
